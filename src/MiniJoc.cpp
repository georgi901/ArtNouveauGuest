...
std::ostream& operator<<(std::ostream& os, const Dificultate& dif) {
    switch(dif) {
        case Dificultate::Usor:  return os << "Usor";
        case Dificultate::Mediu: return os << "Mediu";
        case Dificultate::Greu:  return os << "Greu";
        default:                 return os << "Necunoscut";
    }
}


MiniJoc::MiniJoc(const std::string& nume_joc,
      ...
      finalizat(false)
{
    ...
    switch(dificultate) {
        case Dificultate::Usor:  puncte_maxime = 100; break;
        case Dificultate::Mediu: puncte_maxime = 200; break;
        case Dificultate::Greu:  puncte_maxime = 300; break;
    }
...
int MiniJoc::calculeazaPuncte() {
    ...
    std::cout << "Puncte obținute: " << puncte_obtinute
              << "/" << puncte_maxime << std::endl;
...
std::ostream& operator<<(std::ostream& os, const MiniJoc& joc) {
    os << joc.nume_joc << " (" << joc.getTipJoc() << ")"
       << " - " << joc.dificultate
       << " - " << joc.puncte_obtinute << "/" << joc.puncte_maxime << " puncte"
