/**
 * @file Tablou.cpp
 * @brief Implementarea clasei Tablou.
 *
 * Conține implementarea metodelor pentru afișarea și manipularea tablourilor.
 */
...
Tablou::Tablou(const std::string& titlu, const std::string& tehnica, int an_realizare, bool rar,
               const std::map<std::string, int>& culori, const std::map<std::string, int>& pensule,
               const std::pair<int, int>& dimensiune, const std::shared_ptr<Artist>& artist, bool colectionat)
    : titlu(titlu)
    , tehnica(tehnica)
...
void Tablou::afiseaza_tablou_zoom(NivelDetaliu nivel) const {
    ...
    if (nivel == NivelDetaliu::Complet) {
        ...
        std::cout << "Tehnica: " << tehnica << " | An realizare: " << an_realizare
                  << " | Dimensiune: " << dimensiune.first << "x" << dimensiune.second << " cm\n";
        std::cout << "Colecționat: " << (colectionat ? "Da" : "Nu") << " | Raritate: " << (rar ? "Da" : "Nu") << "\n";
